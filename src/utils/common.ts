...
window.ThemeUtils = {
  'getSongView': () => {
    return new Promise((resolve) => { resolve('compact') })
  },
...
export function humanByteSize(size: number, bitrate = false): string {
  ...
  do {
    parsedSize /= thresh
    ++u
  } while (Math.round(Math.abs(parsedSize) * r) / r >= thresh && u < units.length - 1)
...
export function stripSong(song?: RemoteSong): RemoteSong {
  ...
  if (tmp.album) {
    ...
    if (!tmp.album?.album_coverPath_low?.startsWith('http')) tmp.album.album_coverPath_low = undefined

    if (!tmp.album?.album_coverPath_high?.startsWith('http')) tmp.album.album_coverPath_high = undefined
  }

  if (!tmp.song_coverPath_low?.startsWith('http')) tmp.song_coverPath_low = undefined

  if (!tmp.song_coverPath_high?.startsWith('http')) tmp.song_coverPath_high = undefined
...
export function dotIndex(obj: Record<string, unknown>, is: string | string[], value?: unknown): unknown {
  if (typeof is === 'string') return dotIndex(obj, is.split('.'), value)
  else if (is.length === 1 && value !== undefined) {
    obj[is[0]] === value
    return obj
  } else if (is.length === 0) return obj
  else return dotIndex(obj[is[0]] as Record<string, unknown>, is.slice(1), value)
}
