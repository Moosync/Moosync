...
export default class ThemeHandler extends Vue {
	...
	private async loadCss(cssPath: string) {
		...
		const customStylesheet =
			(document.getElementById("custom-css") as HTMLStyleElement) ??
	...
	private setRGBValues() {
		...
		for (const key of keys) {
			this.root.style.setProperty(
				`--${key}-rgb`,
				this.hexToRgb(docStyle.getPropertyValue(`--${key}`).trim()),
			);
		}
	}
	...
	public fetchThemeFromID() {
		window.ThemeUtils.getActiveTheme().then((theme) =>
	...
	public fetchSongView() {
		window.ThemeUtils.getSongView().then(
			(view) =>
				this.themeStore && (this._themeStore as ThemeStore).songView === view,
		);
	}
