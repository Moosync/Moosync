...
export class YoutubePlayer extends LocalPlayer {
	...
	private async storeCache(id: string, value: unknown) {
		await this.cacheStore.setItem(id, {
	...
	private async getCache<T>(id: string): Promise<T | undefined> {
		const cache = await this.cacheStore.getItem<{ expiry: number; value: T }>(
			id,
		);
	...
	private async getSponsorblock(videoID: string) {
		const preferences =
			await window.PreferenceUtils.loadSelective<Checkbox[]>("audio");
		if (preferences) {
			const sponsorblock = preferences.find(
				(val) => val.key === "sponsorblock",
			);
	...
	protected async _load(src?: string, volume?: number, autoplay?: boolean) {
		...
		if (src) {
			...
			if (videoId) {
				...
				if (this.playerInstance instanceof HTMLAudioElement)
					videoId = await window.SearchUtils.getYTAudioURL(videoId);
			}
		}
		...
		this.playerInstance.ontimeupdate = () => {
			...
			if (time !== lastTime) {
				if (this.currentSegments.length > 0) {
					const segs = this.currentSegments.filter(
						(val) => val.endTime > 1 && val.startTime === Math.floor(time),
					);
					if (segs.length > 0) {
