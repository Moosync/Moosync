...
export class SpotifyProvider extends GenericProvider {
	...
	private getConfig(oauthChannel: string, id: string, secret: string) {
		return {
			...
			scope:
				"playlist-read-private user-top-read user-library-read user-read-private",
	...
	public async updateConfig() {
		const conf = (await window.PreferenceUtils.loadSelective("spotify")) as {
			client_id: string;
			client_secret: string;
		};
		const channel = await window.WindowUtils.registerOAuthCallback(
		...
		if (useUserPass) {
			...
			const username =
				await window.PreferenceUtils.loadSelective<string>("spotify.username");
			...
			if (username && password) {
				try {
					...
					const token = await window.SpotifyPlayer.getToken(
						this._config.scope.split(" ") as unknown[],
					);
	...
	private async populateRequest<K extends ApiResources>(
		...
		if (
			resource === ApiResources.PLAYLIST_ITEMS ||
			resource === ApiResources.PLAYLIST
		) {
			url = resource.replace(
				"{playlist_id}",
				(search as SpotifyResponses.PlaylistItemsRequest).params.playlist_id,
			);
		}
		...
		if (resource === ApiResources.SONG_DETAILS) {
			url = resource.replace(
				"{song_id}",
				(search as SpotifyResponses.TrackItemRequest).params.song_id,
			);
		}
		...
		if (resource === ApiResources.TOP) {
			url = resource.replace(
				"{type}",
				(search as SpotifyResponses.TopRequest).params.type,
			);
		...
		if (
			...
			url = resource.replace(
				"{artist_id}",
				(search as SpotifyResponses.ArtistsTopTracks).params.id,
			);
		}

		if (
			resource === ApiResources.ALBUM_SONGS ||
			resource === ApiResources.ALBUM
		) {
			url = resource.replace(
				"{album_id}",
				(search as SpotifyResponses.AlbumTracksRequest).params.id,
			);
		}
	...
	): Promise<SpotifyResponses.UserDetails.UserDetails | undefined> {
		const validRefreshToken = await this.auth?.hasValidRefreshToken();
		if ((await this.getLoggedIn()) || validRefreshToken) {
			const resp = await this.populateRequest(
				ApiResources.USER_DETAILS,
				{ params: undefined },
				invalidateCache,
			);
	...
	private parseSong(track: SpotifyResponses.PlaylistItems.Track): Song {
		const song: Song = {
			...
			album: {
				...
				album_coverPath_high: track.album.images?.[0]
					? track.album.images?.[0].url
					: "",
			},
			url: track.id,
			song_coverPath_high: track.album.images?.[0]
				? track.album.images?.[0].url
				: "",
	...
	private async parsePlaylistItems(
		items: SpotifyResponses.PlaylistItems.Item[],
	) {
		const parsed: Song[] = [];
		for (const i of items) {
			if (
				!i.is_local &&
				parsed.findIndex((val) => val._id === i.track.id) === -1
			)
	...
	public async validatePlaybackURL(
		playbackUrl: string,
		player: string,
	): Promise<boolean> {
		if (player === "SPOTIFY") {
	...
	public async getPlaybackUrlAndDuration(song: Song, player: string) {
		if (player === "SPOTIFY") {
			...
			if (ytItem) {
				return {
					url: ytItem.playbackUrl ?? ytItem.url,
	...
	public matchSongUrl(url: string) {
		return !!url.match(
			/^(https:\/\/open.spotify.com\/(track|embed)\/|spotify:track:)([a-zA-Z0-9]+)(.*)$/,
		);
	}

	public async getSongDetails(
		url: string,
		_ = false,
	...
	private parseRecommendations(
		recommendations: SpotifyResponses.RecommendationDetails.Recommendations,
	) {
		const songList: Song[] = [];
	...
	public async *getRecommendations(): AsyncGenerator<Song[]> {
		if (await this.getLoggedIn()) {
			...
			seedTracks.push(
				...libraryTracks.map((val) => val._id.replace("spotify:", "")),
			);
			...
			const recommendationsArtistsResp = await this.populateRequest(
				ApiResources.RECOMMENDATIONS,
				{
					params: {
			...
			const recommendationsTracksResp = await this.populateRequest(
				ApiResources.RECOMMENDATIONS,
				{
					params: {
	...
	private parseArtist(
		artist: SpotifyResponses.RecommendationDetails.SpotifyArtist,
	): Artists {
		return {
			artist_id: `spotify-author:${artist.id}`,
			artist_name: artist.name,
	...
	private async *_getAlbumSongs(
		album: SpotifyResponses.RecommendationDetails.Album,
	) {
		if (await this.getLoggedIn()) {
			let nextOffset = 0;
			let isNext = false;
		...
		if (artist.artist_name) {
			...
			if (resp.length > 0) {
				window.DBUtils.updateArtist({
					...
					artist_coverPath:
						artist.artist_coverPath ?? resp?.[0].artist_coverPath,
					artist_extra_info: {
	...
	public async *getAlbumSongs(
		...
		if (await this.getLoggedIn()) {
			if (album.album_name) {
				...
				if (!albumId) {
					albumId = (await this.searchAlbum(album.album_name))?.[0]
						?.album_extra_info?.spotify?.album_id;
				}
	...
	public async getSongById(id: string): Promise<Song | undefined> {
		if (this.matchEntityId(id)) {
			...
			const song = this.getSongDetails(
				`https://open.spotify.com/track/${sanitized}`,
			);
			return song;
		}
		return;
