...
export class YoutubeProvider extends GenericProvider {
	...
	public async updateConfig(): Promise<boolean> {
		const conf = (await window.PreferenceUtils.loadSelective("youtube")) as {
			client_id: string;
		...
		if ((conf?.client_id && conf.client_secret) || this.isEnvExists()) {
			const channel =
				await window.WindowUtils.registerOAuthCallback("ytoauth2callback");
			console.log(channel);
	...
	private async parsePlaylists(
		items: YoutubeResponses.UserPlaylists.Item[],
	...
	public async *getPlaylistContent(
		...
		if (id) {
			if (await this.getLoggedIn()) {
				...
				const parsed = await this.parsePlaylistItems(
					resp.items,
					invalidateCache,
				);
				yield { songs: parsed, nextPageToken: resp.nextPageToken };
			} else {
				yield window.SearchUtils.getYTPlaylistContent(
					id,
					convertProxy(nextPageToken) as never,
				);
			}
		}
		return;
	...
	private async parseVideo(
		items: { item: YoutubeResponses.VideoDetails.Item; date?: string }[],
	) {
		const songs: Song[] = [];
		for (const v of items) {
			if (
				v.item.id &&
				songs.findIndex((value) => value._id === v.item.id) === -1
			)
				songs.push({
					...
					artists: [
						{
							...
							artist_name: v.item.snippet.channelTitle
								.replace("-", "")
								.replace("Topic", "")
								.trim(),
							artist_extra_info: {
	...
	private async getSongDetailsFromID(
		invalidateCache: boolean,
		...songs: { id?: string; date?: string }[]
	) {
		const validRefreshToken = await this.auth?.hasValidRefreshToken();
		...
		if (filtered.length > 0) {
			if ((await this.getLoggedIn()) || validRefreshToken) {
				...
				if (filtered.length !== resp.items.length) {
					console.warn(
						"Something went wrong while parsing song details. Length mismatch",
					);
				}
				...
				for (let i = 0; i < resp.items.length; i++) {
					items.push({
						item: resp.items[i],
						date: filtered[i].date ?? resp.items[i].snippet.publishedAt,
					});
				}
			...
			if (parsedFromURL) {
				...
				try {
					...
					if (resp.items) {
						...
						songList.push(
							...(await this.getSongDetailsFromID(false, ...finalIDs)),
						);
					}
				} catch (e) {
					console.error(
						"Youtube search api failed. Falling back to unofficial search",
						e,
					);
	...
	public async getSongDetails(
		...
		if (this.matchSongUrl(url)) {
			...
			if (videoID) {
				const details = await this.getSongDetailsFromID(invalidateCache, {
					id: videoID,
				});
				...
				const scraped = await window.SearchUtils.searchYT(
					videoID,
					undefined,
					false,
					true,
		...
		for (const s of selected) {
			const song = (
				await this.searchSongs(
					`${
						s.artists
							? `${s.artists?.map((val) => val.artist_name).join(", ")} - `
							: ""
					}${s.title}`,
				)
			)[0];

			const recommendations = await window.SearchUtils.getYTSuggestions(
				this.sanitizeId(song._id, "SONG"),
			);
	...
	public async *getRecommendations(): AsyncGenerator<Song[]> {
		...
		for (const song of youtubeSongs.slice(0, 10)) {
			const songs = await window.SearchUtils.getYTSuggestions(
				this.sanitizeId(song._id, "SONG"),
			);
			count += songs.length;
			yield songs;
		...
		if (await this.getLoggedIn()) {
			if (count < 10) {
				...
				(
					...
					resp.push({ id: val.id.videoId, date: val.snippet.publishedAt }),
				);
			}
		...
		if (await this.getLoggedIn()) {
			...
			if (channelId) {
				...
				while (finalIDs.length > 0) {
					yield {
						songs: await this.getSongDetailsFromID(
							false,
							...finalIDs.splice(0, 50),
						),
			...
			if (channelId) {
				yield await window.SearchUtils.getYTPlaylistContent(
					channelId,
					convertProxy(nextPageToken as never, true),
				);
			} else {
				const resp = await window.SearchUtils.searchYT(`${artist.artist_name}`);
				if (
					resp.artists?.length > 0 &&
					resp.artists[0].artist_extra_info?.youtube?.channel_id
				) {
					yield await window.SearchUtils.getYTPlaylistContent(
						resp.artists[0].artist_extra_info.youtube.channel_id,
					);
	...
	private parseArtist(
		artist: YoutubeResponses.ChannelInfo.ChannelInfo,
	): Artists | undefined {
	...
	public async getArtistDetails(artist: Artists) {
		if (await this.getLoggedIn()) {
			if (artist.artist_extra_info?.youtube?.channel_id) {
				const artistDetails = await this.populateRequest(
					ApiResources.CHANNELS,
					{
						params: {
	...
	public async searchArtists(term: string): Promise<Artists[]> {
		...
		if (await this.getLoggedIn()) {
			...
			const resp = await window.SearchUtils.searchYT(
				term,
				undefined,
				false,
				false,
	...
	private parseSearchPlaylists(
		...items: YoutubeResponses.SearchDetails.Item[]
	) {
	...
	public async getPlaybackUrlAndDuration(
		...
		if (!song.duration && song.url) {
			const fetchedSong = await this.getSongDetails(
				`https://youtube.com/watch?v=${song.url}`,
			);
