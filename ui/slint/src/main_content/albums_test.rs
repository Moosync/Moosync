...
use crate::{
    AlbumsPageProps, MainWindow, main_content::albums::AlbumsPageHandler, pages::PageHandler,
...
fn test_albums_page_handler_on_show() {
    run_async_test(|| async move {
        let tmp = TempDir::new("moosync_ui_albums_show").unwrap();
...
#[test]
#[tracing::instrument(level = "debug", skip_all)]
fn test_albums_page_handler_on_hide() {
    run_async_test(|| async move {
        let tmp = TempDir::new("moosync_ui_albums_hide").unwrap();
        let test_dir = tmp.path().to_path_buf();
        let context = PluginContext {
            data_dir: test_dir.clone(),
            cache_dir: test_dir.clone(),
