...
use crate::{
    AllSongsPageProps, ContextMenuCallbacks, MainWindow,
    error::UiError,
    pages::PageHandler,
...
impl<'a> AllSongsPageHandler<'a> {
    ...
    async fn fetch_songs(state_manager: &StateManager) -> Result<Vec<Song>, UiError> {
        tracing::debug!("Fetching all songs from database");
    ...
    fn set_songs(main_window: &MainWindow, state_manager: &StateManager, songs: Vec<Song>) {
        ...
        let model = make_lazy_song_model(main_window, state_manager, songs_view);
        main_window.global::<AllSongsPageProps>().set_songs(model);
    }
}
...
impl<'a> PageHandler for AllSongsPageHandler<'a> {
        ...
        tokio::spawn({
            ...
            async move {
                match Self::fetch_songs(&state_manager).await {
                    ...
                    Err(e) => {
                        tracing::error!("AllSongsPage: Failed to fetch all songs: {:?}", e);
                    }
                }
            }
            .in_current_span()
    ...
    #[tracing::instrument(level = "debug", skip_all)]
    fn on_hide(&self) {
        tracing::debug!("AllSongsPage: on_hide clearing songs");
        self.main_window
