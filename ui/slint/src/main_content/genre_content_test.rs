...
use crate::{
    GenreContentPageProps, MainWindow, main_content::genre_content::GenreContentPageHandler,
...
fn test_genre_content_page_handler_on_show() {
    run_async_test(|| async move {
        let tmp = TempDir::new("moosync_ui_genre_content_show").unwrap();
...
fn test_genre_content_page_handler_on_hide() {
    run_async_test(|| async move {
        let tmp = TempDir::new("moosync_ui_genre_content_hide").unwrap();
        let test_dir = tmp.path().to_path_buf();
        let context = PluginContext {
            data_dir: test_dir.clone(),
            cache_dir: test_dir.clone(),
