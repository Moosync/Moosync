...
use crate::{
    GenresPageProps, MainWindow, main_content::genres::GenresPageHandler, pages::PageHandler,
    test_utils::run_async_test,
};

#[test]
#[tracing::instrument(level = "debug", skip_all)]
fn test_genres_page_handler_on_show() {
    run_async_test(|| async move {
        let tmp = TempDir::new("moosync_ui_genres_show").unwrap();
...
#[test]
#[tracing::instrument(level = "debug", skip_all)]
fn test_genres_page_handler_on_hide() {
    run_async_test(|| async move {
        let tmp = TempDir::new("moosync_ui_genres_hide").unwrap();
        let test_dir = tmp.path().to_path_buf();
        let context = PluginContext {
            data_dir: test_dir.clone(),
            cache_dir: test_dir.clone(),
