...
use crate::{
    AppCallbacks, MainWindow, PlaylistContentPageProps, PlaylistsPageProps, SongModel,
    error::UiError,
    pages::PageHandler,
    utils::{
    ...
    fn toggle_extension(
        ...
        tokio::spawn({
            ...
            async move {
                ...
                let Ok(new_songs) =
                    Self::fetch_extension_songs(&state_manager, playlist, package_name.clone())
                        .await
                else {
                    tracing::error!(
                        "Failed to fetch playlist songs from extension {}",
                        package_name
                    );
                    let _ = weak.upgrade_in_event_loop(move |window| {
                        Self::update_providers_enabled(&window, &package_name, false);
    ...
    fn on_show(&self) {
        ...
        tokio::spawn({
            ...
            async move {
                ...
                let Ok(songs) = Self::fetch_songs(&state_manager, playlist, extension).await else {
                    tracing::error!("Failed to fetch playlist songs");
                    return;
                };
