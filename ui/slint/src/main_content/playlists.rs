...
use songs_proto::moosync::types::{
    Playlist, PlaylistList, GetEntityOptions, entity_result,
};
...
async fn run_scanner_loop(
    ...
    while let Some(p) = progress.recv().await {
        if p == ScanProgress::STOPPED {
            fetch_and_cache_playlists(main_window_weak.clone(), state_manager.clone(), playlists_cache.clone()).await;
        }
    }
}
...
async fn get_playlists_from_db(
    ...
    match playlists_res.result {
        ...
        _ => Err(MoosyncError::String("Failed to get playlists from db".to_string())),
    }
}
...
fn set_all_playlists(main_window: &MainWindow, playlists: Vec<Playlist>) {
    ...
    let playlist_model = playlists
        ...
        .map(|playlist| PlaylistModel {
            ...
            coverPathUrl: playlist.playlist_coverpath.clone().unwrap_or_default().into(),
