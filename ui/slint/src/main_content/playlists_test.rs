...
use crate::{
    MainWindow, PlaylistsPageProps, main_content::playlists::PlaylistsPageHandler,
    pages::PageHandler, test_utils::run_async_test,
};

#[test]
#[tracing::instrument(level = "debug", skip_all)]
fn test_playlists_page_handler_on_show() {
    run_async_test(|| async move {
        let tmp = TempDir::new("moosync_ui_playlists_show").unwrap();
...
#[test]
#[tracing::instrument(level = "debug", skip_all)]
fn test_playlists_page_handler_on_hide() {
    run_async_test(|| async move {
        let tmp = TempDir::new("moosync_ui_playlists_hide").unwrap();
        let test_dir = tmp.path().to_path_buf();
        let context = PluginContext {
            data_dir: test_dir.clone(),
            cache_dir: test_dir.clone(),
