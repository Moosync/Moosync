...
pub fn generate_tauri_invoke_wrapper(file_path: &Path, valid_crates: Vec<String>) {
    ...
    .replace(
        ...
        "crate::utils::error::MoosyncError",
    )
    .replace("types :: errors :: Result", "crate::utils::error::Result");

    let out_dir = env::var("OUT_DIR").expect("OUT_DIR environment variable is not set");
    let dest_path = Path::new(&out_dir).join("invoke");
    if !dest_path.exists() {
        fs::create_dir_all(&dest_path).expect("Failed to create output directory");
    }
    let dest_path = dest_path.join("mod.rs");
    fs::write(&dest_path, output).expect("Failed to write generated tauri_invoke.rs");
}
...
fn parse_fn(dets: &FnDetails, valid_crates: &Vec<String>) -> proc_macro2::TokenStream {
    ...
    let ret_type = if let Some(ret) = &dets.ret {
        ...
        if !is_valid_type(&parsed_ret_type, valid_crates) {
            ...
            is_ret_value = true;
            parsed_ret_type =
                syn::parse_str::<syn::Type>("crate::utils::error::Result<wasm_bindgen::JsValue>")
                    .unwrap();
        }
